...
header {
  position: relative;
  z-index: 1001; /* Assurez-vous que le header est au-dessus de l'overlay */
}
...
.page-overlay {
  ...
  left: 0;
  width: 100%;
  height: calc(100% - 80px); /* Ajustez également selon la hauteur de votre header */
  background: rgba(0, 0, 0, 0.8);
  color: #fff;
  display: flex;
  align-items: center;
  justify-content: center;
  z-index: 1000; /* Overlay en-dessous du header */
}
...
#overlay {
    ...
    z-index: 1000; /* Assurez-vous que c'est inférieur à la modale */
    display: none;
}
...
.modal {
    position: fixed;
    z-index: 2000; /* Plus haut que l'overlay */
}
...
/* Texte barré pour les tâches validées */
.completed-task {
    text-decoration: line-through;
    color: #888; /* Pour que le texte barré apparaisse plus subtilement */
}
...
.project-details-inline {
  padding: 15px;
  margin-top: 20px; /* Espace entre le bloc et le header */
}

.project-details-inline h3 {
  margin-bottom: 15px; /* Espace entre le titre et les champs */
}
...
.project-field label {
  min-width: 120px; /* Ajustez pour aligner les libellés */
  color: var(--text-dark);
}
...
/* Largeur spécifique pour chaque champ */
#projectTitle {
  width: 250px; /* Augmentez pour un champ plus long */
}

#projectStart,
#projectEnd {
  width: 120px; /* Réduisez pour des champs de date plus courts */
}

#projectBudget {
  width: 150px; /* Taille standard pour le champ budget */
}
...
.group-header-content {
  flex: 1; /* Pour que le contenu prenne l'espace disponible */
}
...
.btn-toggle-mode:hover {
    background-color: #3d5079;
      ...
      /* Style pour le bouton de commentaire */
.btn-comment-toggle {
    background-color: var(--secondary-dark);
    color: var(--text-dark);
    border: none;
...
/* Style pour la section de commentaire */
.comment-section {
    margin-top: 10px;
}

/* Vos styles existants pour .comment-box */
.comment-box {
...
.group-progress-container {
  ...
  background-color: var(--border-dark);
  border-radius: 8px; /* Un peu plus petit que la barre globale */
  margin: 5px 0; /* Réduire la marge */
}

.group-progress-bar {
  height: 16px; /* Un peu plus petit que la barre globale */
  border-radius: 8px;
  transition: width 0.3s ease, background-color 0.3s ease;
  display: flex;
  align-items: center;
    ...
    <div class="container">
        <div class="header-content">
            ...
            <div class="left-section">
                <svg class="app-logo" width="50" height="50" viewBox="0 0 100 100">
                    <defs>
...
<div id="overlay" class="overlay">
  ...
  <h3>Fiche Projet</h3>
  <div class="project-fields-container" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 20px;">
    <div class="project-field">
      <label for="projectTitle">Nom du projet</label>
      <input type="text" id="projectTitle" value="" style="width: 100%;">
    </div>
    <div class="project-field">
      <label for="projectStart">Date de début</label>
      <input type="date" id="projectStart" value="" style="width: 100%;">
    </div>
    <div class="project-field">
      <label for="projectEnd">Date de fin</label>
      <input type="date" id="projectEnd" value="" style="width: 100%;">
    </div>
    <div class="project-field">
      <label for="projectBudget">Budget alloué (€)</label>
      <input type="number" id="projectBudget" value="0" min="0" style="width: 100%;" onchange="updateProjectBudget(this.value)">
    </div>
  </div>
</div>
  ...
  <div class="modal-content">
    ...
    <form id="userUpdateForm" onsubmit="updateUserInfo(event)">
      ...
      <div class="form-group">
        ...
        <input type="email" id="userEmail" name="userEmail" value="<?php echo htmlspecialchars($userEmail); ?>" disabled>
      </div>
      <div class="form-group">
...
function toggleTaskValidation(taskId) {
  ...
  <div class="group-header-content">
    <h3>${groupByCategory ? '' : 'Phase'}  ${key} <span class="budget-text">${formatNumber(groupBudget)} €</span></h3>
    ...
    const validationIconClass = task.completed ? 'fas fa-check-circle validated' : 'far fa-circle';
        ...
        taskElement.innerHTML = ` 
																			<div class="task-header">
																			    <div class="task-header-left" style="display: flex; align-items: center; gap: 10px;">
                ...
                <!-- Libellé de la tâche avec largeur fixe pour le conteneur et 100% pour les éléments internes -->
                <div class="task-name" id="task-name-container-${task.id}" style="width: 800px;">
                    ${task.completed 
                        ? `<span style="text-decoration: line-through; color: #888; display: inline-block; width: 100%; font-weight: normal;">${task.name}</span>`
                        : `<input type="text" value="${task.name}" class="task-field" data-task-id="${task.id}" data-attribute="libelle" style="width: 100%; font-weight: normal;">`
                    }
                </div>
            </div>
...
function toggleTaskDetails(taskId) {
    ...
    if (progressElement) {
        progressElement.textContent = `${formatNumber(progresst)} h`;
    }
